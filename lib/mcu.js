...
const wrtcclient = require('./webrtcconnection.js').wrtcclient;
...
function isAnInterpreterInOccupiedRoom(roomName){
...
function closeOccupiedRoom(roomName, socketid){
    ...
    console.log('closeOccupiedRoom socketid leaving', socketid);
    if (roomIndex >= 0){
        console.log('closeOccupiedRoom roomIndex', roomIndex);
        let users = ACD_occupied[roomIndex].users;
        ...
        users.forEach( user => {
            if (user.type == 'interpreter'){
                ...
                console.log('closeOccupiedRoom -- looping through interpreters: socket.id', terpUser.socketid);
                console.log('closeOccupiedRoom -- socket.id match?', terpUser.socketid === socketid);

                // if the user leaving is an interpreter, do not allow back into ACD queue
                if (socketid === terpUser.socketid){
                } else {
                    console.log('pushing interpreter back to ACD Queue', terpUser.socketid);
        ...
        console.log('ACD_occupied room index not found', roomIndex);
    }
}

function interpreterExitACD(socketid){
    console.log('interpreterExitACD socketid', socketid);
    let terpIndex = ACD_interpreter_list.findIndex( user => {
        console.log('interpreterExitACD user.socketid', user.socketid);
        return user.socketid == socketid;
    });

    console.log('interpreterExitACD terpIndex', terpIndex);

    if (terpIndex >= 0){
        ACD_interpreter_list.splice(terpIndex, 1);
        console.log('ACD_interpreter_list.length', ACD_interpreter_list.length);
    }
}
...
function joinTranscriberRoom(socket, stunOn){
    ...
    updateIce(stunOn, (iceconfig) => {
        socket.emit('createTranscriberPeer', {
            user: 'some_id_generated_from_webrtcserver',
            ice: iceconfig
        });
    });
...
function handleJoinRoom(socket){
    socket.on('join', function (message) {
        ...
        console.log('handleJoinRoom -- interpreter quantity vacant:', ACD_interpreter_list.length);
        console.log('handleJoinRoom -- interpreter quantity occupied:', ACD_occupied.length);
        if (userType === "interpreter"){
            socket.emit('id', {yourId:socket.id});
            ACD_interpreter_list.push({roomName: roomName, socketid: socket.id, stunOn: stun});
            joinRoom(socket, roomName, stun);
        } else if (userType === "patron"){

            let videoEnabled = message.videoEnabled ? message.videoEnabled : false;
            let transcriberEnabled = message.transcriberEnabled ? message.transcriberEnabled : false;
            ...
            if (videoEnabled && transcriberEnabled){
                ...
                if(ACD_interpreter_list.length < 1){
                    ...
                    socket.emit('id', {yourId:socket.id});
                    roomName = createOccupiedRoom(socket.id,stun);
                    console.log('handleJoinRoom -- roomName:', roomName, 'userType:', userType);
                    joinRoom(socket, roomName, stun);
                }

                joinTranscriberRoom(socket, stun);
                ...
                // until transcriber API is hooked up
                socket.emit('info', {info:'novacancy'});
